import getLendersRecipe from '../fixtures/lenders';

context('Home Page', () => {
  let lenders = null;
  before(() => {
    lenders = getLendersRecipe.default();
  });
  ...
  it('will check if all important elements are in place:', () => {
    // Names and links for each banck should be visible for the user:
    lenders.forEach((lender) => {
      cy.get(`[data-cy="${lender.slug}"]`)
        .should('be.visible')
  ...
  it('will check if all important links are in place and take the user to correct URL', () => {
    ...
    cy.get(`[data-cy="${lenders[0].slug}"]`).click();
    cy.location('pathname').should('be', `/${lenders[0].slug}`);
    cy.go('back');

    cy.get(`[data-cy="${lenders[1].slug}"]`).click();
    cy.location('pathname').should('be', `/${lenders[1].slug}`);
    cy.go('back');

    cy.get(`[data-cy="${lenders[2].slug}"]`).click();
